...
def main():
    ...
    # Pressure kPa unit, Resistance ohm unit, watermark mm unit
    pressure_values = pd.read_csv('Cal_Watermark_Datasheet.csv')['Pressure'].tolist()
    resistance_values = pd.read_csv('Cal_Watermark_Datasheet.csv')['Resistance'].tolist()
    ...
    # Plot the results
    plt.figure()
    plt.plot(crono, mat_pot, color='#2e8b57')
    plt.xlabel("time")
    plt.ylabel("pressure")
    plt.show()
